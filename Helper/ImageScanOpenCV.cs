...
namespace KAutoHelper
{
    ...
    public class MotionDetection
    {
        ...
        public static void InitializeMotionDetection()
        {
            timer = new System.Timers.Timer(100); // Chụp hình mỗi 100ms
        ...
        private static void ProcessFrame(object sender, EventArgs e)
        {
            ...
            Bitmap currentBitmap = CaptureScreen();

            if (currentBitmap != null)
            {
                // Chuyển đổi hình ảnh sang Mat và ảnh xám
                Mat currentFrame = BitmapToMat(currentBitmap);
                ...
                if (prevGrayFrame != null)
                {
                    ...
                    double threshold = 30;
                    CvInvoke.Threshold(diffFrame, diffFrame, threshold, 255, ThresholdType.Binary);

                    using (var contours = new Emgu.CV.Util.VectorOfVectorOfPoint())
                    {
                        CvInvoke.FindContours(diffFrame, contours, null, RetrType.External, ChainApproxMethod.ChainApproxSimple);

                        if (contours.Size > 0)
                        {
                            Point motionLocation = contours[0][0];
                            // Thực hiện lệnh click chuột tại motionLocation ở đây
                        }
                    }
        ...
        {
            // Tìm cửa sổ LDPlayer bằng tên
            IntPtr hWnd = FindWindow(null, "LDPlayer");
            if (hWnd == IntPtr.Zero)
            {
                return null;
            }
        ...
        private static Mat BitmapToMat(Bitmap bitmap)
        {
            ...
            // Chuyển đổi từ Bitmap sang Image<Gray, Byte>
